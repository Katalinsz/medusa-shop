  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Dimensions Badge */}
        {width && height && (
          <div className="absolute top-3 left-3 px-2 py-1 bg-emerald-600/90 text-white text-xs font-semibold rounded-full backdrop-blur-sm">
            {width}×{height}
          </div>
        )}
      ...
      {/* Card Content - Consistent height regardless of image size */}
      <div
        className={`
        p-4 flex flex-col border-t border-gray-100 flex-shrink-0
        ${viewMode === "list" ? "flex-1 justify-center" : ""}
        ${cardSizeClass === "small-card" ? "min-h-[80px]" : "min-h-[90px]"}
      `}
      >
        <Link href={`/products/${product.handle}`} className="flex-1 group">
          <h3 className="font-semibold text-gray-900 mb-1 line-clamp-2 group-hover:text-emerald-600 transition-colors text-sm leading-tight">
            {product.title}
          </h3>
        ...
        {/* Bottom row with price and artist */}
